...
const createCursorDecorations = (otherCursors: { [sid: string]: { cursor_position: number; color: string } }): Extension => {
    return EditorView.decorations.of(view => {
        const builder = new RangeSetBuilder<Decoration>();
        for (const [sid, cursor] of Object.entries(otherCursors)) {
            const { cursor_position, color } = cursor;
            if (typeof cursor_position === 'number') { // Ensure cursor_position is a number
...
const CodeEditor: React.FC = () => {
    ...
    const handleLanguageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (["python", "cpp", "javascript", "java"].includes(newLanguage)) {
            ...
            if (joinedRoom) { // Emit only if joined
    ...
    const throttledEmitCursor = useMemo(() => {
        return throttle((position: number) => {
            if (joinedRoom) { // Ensure joined before emitting
